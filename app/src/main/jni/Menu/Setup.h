...
void Init(JNIEnv *env, jobject thiz, jobject ctx, jobject title, jobject subtitle){
    //Set sub title
    setText(env, title, OBFUSCATE("<b>Nepmods VIP Tester V3</b>"));

    //Set sub title
    setText(env, subtitle, OBFUSCATE("<b>Nepmods</b>"));

    //Dialog Example
    //setDialog(ctx,env,OBFUSCATE("Nepmods"),OBFUSCATE("This Tool Is Made By Nepmods"));

    //Toast Example
    Toast(env,ctx,OBFUSCATE("Nepmods"),ToastLength::LENGTH_LONG);
